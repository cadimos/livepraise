...
import { migrateTabVerses, musicVersesForExport, queueItemsFromExport, queueItemsForExport, } from './queue-items.js';
export const PLAYLIST_FORMAT = 'livepraise-playlist';
export const PLAYLIST_FORMAT_VERSION = 1;
function tabQueueItems(tab) {
    if (tab.items?.length)
        return tab.items;
    if (tab.verses?.length)
        return migrateTabVerses(tab.verses);
    return [];
}
function parseQueueItemExport(raw) {
    ...
    if (typeof item.songName === 'string')
        parsed.songName = item.songName;
    if (typeof item.artist === 'string')
        parsed.artist = item.artist;
    if (typeof item.bibleFile === 'string')
        parsed.bibleFile = item.bibleFile;
    if (typeof item.bookId === 'number' && Number.isFinite(item.bookId)) {
        parsed.bookId = item.bookId;
    }
    if (typeof item.bookName === 'string')
        parsed.bookName = item.bookName;
    ...
    if (typeof item.mediaPath === 'string')
        parsed.mediaPath = item.mediaPath;
    if (typeof item.thumbPath === 'string')
        parsed.thumbPath = item.thumbPath;
    if (typeof item.youtubeVideoId === 'string') {
        parsed.youtubeVideoId = item.youtubeVideoId;
    }
    return parsed;
}
export function buildPlaylistExport(tabs) {
    const items = tabs
        .map((tab, index) => {
        const queue = tabQueueItems(tab);
        const verses = tab.verses?.map((v) => ({ id: v.id, text: v.text })) ??
...
export function playlistExportHasContent(tabs) {
    return tabs.some((tab) => tabQueueItems(tab).length > 0);
}
export function parsePlaylistExport(raw) {
    let data;
    try {
        data = JSON.parse(raw);
    }
    ...
    if (file.format !== PLAYLIST_FORMAT) {
        throw new Error(`Formato não reconhecido (esperado "${PLAYLIST_FORMAT}").`);
    }
    if (file.version !== PLAYLIST_FORMAT_VERSION) {
        throw new Error(`Versão não suportada (esperado ${PLAYLIST_FORMAT_VERSION}, recebido ${String(file.version)}).`);
    }
    if (!Array.isArray(file.items) || !file.items.length) {
        throw new Error('Playlist vazia ou sem itens.');
    }
    const items = [...file.items].sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
    const normalized = [];
...
export function mapResolveBySongId(entries) {
    const map = new Map();
...
function importTabBase(item, extra = {}) {
...
export function buildImportTabsFromExport(file, resolved, versesBySongId, missingMessage) {
        ...
        if (item.queueItems?.length) {
            ...
            if (hit?.exists) {
                ...
                return importTabBase(item, {
                    ...
                    verses: versesBySongId.get(songId) ??
