...
export function clampMaxEstofreLines(value) {
    const n = Math.round(Number(value));
    if (!Number.isFinite(n))
        return DEFAULT_MAX_ESTOFRE_LINES;
...
export function splitVerseLinesForDisplay(lines, maxLinesPerEstofre) {
...
export function splitVerseTextForDisplay(text, maxLinesPerEstofre) {
    const normalized = normalizeVerseText(text);
    if (!normalized.trim())
        return [];
    const lines = normalized.split('\n');
    return splitVerseLinesForDisplay(lines, maxLinesPerEstofre).map((chunk) => chunk.join('\n'));
}
/** Expande versos da BD em blocos de exibição (ids sintéticos estáveis por verso). */
export function expandVersesForDisplay(verses, maxLinesPerEstofre) {
    const max = clampMaxEstofreLines(maxLinesPerEstofre);
    const out = [];
