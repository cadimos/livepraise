...
export function parseYouTubeVideoId(input: string): string | null {
  ...
  for (const pattern of URL_PATTERNS) {
    ...
    if (match?.[1] && isValidYouTubeVideoId(match[1])) return match[1];
  }
  return null;
}

export interface YoutubeEmbedOptions {
  /** Origem da página hospedeira — exigida pelo player em alguns ambientes (ex. Electron). */
  origin?: string;
  /** URL da página que embute o player (Referer exigido pelo YouTube desde 2025). */
