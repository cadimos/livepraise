...
export function formatComboLabel(combo) {
    if (!combo)
        return '—';
    const parts = [];
    if (combo.ctrl)
...
export function findShortcutConflict(shortcuts, shortcutId, combo) {
    return (shortcuts.find((item) => item.id !== shortcutId
        && item.combo
        && combosEqual(item.combo, combo)) ?? null);
}
