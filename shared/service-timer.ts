...
export const SERVICE_TIMER_VERSION = 1 as const;

export type ServiceTimerMode = 'counter' | 'timer';

export type ServiceTimerTargetKind = 'display' | 'external';

export interface ServiceTimerTarget {
  kind: ServiceTimerTargetKind;
  /** displayId numérico ou deviceId UUID */
  id: string;
  mode: ServiceTimerMode;
}

export interface ServiceTimerState {
  version: typeof SERVICE_TIMER_VERSION;
  /** Ferramenta activa no operador (mostra controlos). */
  active: boolean;
  running: boolean;
  /** Epoch ms do último arranque (null se parado). */
  startedAt: number | null;
  /** Tempo já decorrido antes da pausa actual (ms). */
  accumulatedMs: number;
  /** Duração do countdown para alvos em modo timer (ms). */
...
export function defaultServiceTimerState(): ServiceTimerState {
  return {
...
function parseTarget(raw: unknown): ServiceTimerTarget | null {
...
export function parseServiceTimerState(raw: unknown): ServiceTimerState | null {
  if (!isRecord(raw)) return null;
  if (raw.version !== SERVICE_TIMER_VERSION) return null;

  const targets: ServiceTimerTarget[] = [];
...
export function encodeServiceTimerState(state: ServiceTimerState): string {
  return JSON.stringify(state);
}

export function decodeServiceTimerValor(valor: string): ServiceTimerState | null {
  if (!valor.trim()) return defaultServiceTimerState();
  try {
...
export function counterElapsedMs(state: ServiceTimerState, now = Date.now()): number {
...
export function timerRemainingMs(state: ServiceTimerState, now = Date.now()): number {
...
export function displayMsForMode(
...
export function findTargetForClient(
  state: ServiceTimerState,
  kind: ServiceTimerTargetKind,
  id: string,
...
export function shouldShowServiceTimerOnClient(
  state: ServiceTimerState,
  kind: ServiceTimerTargetKind,
  id: string,
): boolean {
...
export function startServiceTimer(state: ServiceTimerState, now = Date.now()): ServiceTimerState {
  if (state.running) return state;
  return { ...state, running: true, startedAt: now };
}

export function pauseServiceTimer(state: ServiceTimerState, now = Date.now()): ServiceTimerState {
  if (!state.running) return state;
  const elapsed = counterElapsedMs(state, now);
  return {
    ...state,
...
export function resetServiceTimer(state: ServiceTimerState): ServiceTimerState {
  return {
    ...state,
    running: false,
    startedAt: null,
