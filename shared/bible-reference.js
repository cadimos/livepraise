...
function pickBestMatch(candidates, bookQuery) {
...
function findBookByNormalizedName(books, norm) {
...
export function findBookByReference(books, bookQuery) {
    const q = normalizeBibleText(bookQuery);
    if (!q || books.length === 0)
        return null;
...
export function findBookFallbackForReference(books, bookQuery, primary, chapter, maxChapters) {
    const q = normalizeBibleText(bookQuery);
    if (q !== 'jo' || normalizeBibleText(primary.nome) !== 'jo')
        return null;
    if (chapter == null || maxChapters == null)
        return null;
