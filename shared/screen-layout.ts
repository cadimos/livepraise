...
export const SCREEN_CONTENT_FITS = ['estender', 'centralizar', 'proporcional'] as const;
...
export function normalizeContentFit(value: string): ScreenContentFit {
  return (SCREEN_CONTENT_FITS as readonly string[]).includes(value)
    ? (value as ScreenContentFit)
    : 'estender';
}
...
export function buildAjustarTelaValor(
  ...
  if (preset === 'personalizado') {
    ...
    if (custom) return `${custom.w}x${custom.h}`;
    return '0x0';
  }
  return preset;
}
...
export function describeScreenLayoutSize(
  ...
  if (!preset || preset === 'padrao') return 'padrao';
  if (preset === 'personalizado') {
    const custom = parseCustomScreenPixels(largura, altura);
...
type ScreenLayoutInput = ScreenLayoutFields & {
  preset: string;
  largura: string;
  altura: string;
};

function buildAjustarTelaPayloadForTarget(
  targetId: number | string,
  screen: ScreenLayoutInput,
): string {
