...
export function clampBibleSearchHistoryLimit(value) {
    const n = Math.round(Number(value));
    if (!Number.isFinite(n))
        return BIBLE_SEARCH_HISTORY_DEFAULT_LIMIT;
    return Math.min(BIBLE_SEARCH_HISTORY_MAX_LIMIT, Math.max(BIBLE_SEARCH_HISTORY_MIN_LIMIT, n));
}
/** Últimas pesquisas (mais recente primeiro), sem duplicados case-insensitive. */
export function pushBibleSearchHistoryEntry(history, query, limit) {
...
export function sanitizeBibleSearchHistory(history, limit) {
