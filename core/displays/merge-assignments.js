...
export function inferDefaultRoleForNewDisplay(displayId, primaryDisplayId, connected, savedAssignments) {
    if (displayId === primaryDisplayId)
        return 'operator';
    const activeRoles = new Set(savedAssignments
        .filter((a) => connected.some((d) => d.id === a.displayId))
        .map((a) => a.role));
    if (!activeRoles.has('projection'))
        return 'projection';
    if (!activeRoles.has('stage-return'))
        return 'stage-return';
    return 'off';
}
/**
 ...
 */
export function mergeDisplayAssignments(connected, saved, primaryDisplayId) {
    const savedList = saved ?? [];
    const byId = new Map(connected.map((d) => [d.id, d]));
    const assignments = connected.map((display, index) => {
