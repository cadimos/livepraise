...
module.exports = app => {
    ...
    api.post("/:codigo", async (req, res, next) => {
        let codigo = req.params.codigo;
        var errors = []
        if (!req.body.cat) {
            errors.push("Categoria Obrigatória");
        ...
        await db.run("UPDATE musica SET cat=?, nome=?, nome2=?, artista=?,compositor=? WHERE id=?", [data.cat, data.nome, data.nome, data.artista, data.compositor, codigo]);
